const api = require('./juros')

test('jurosSimples', () => {
  const C = 100
  const i = 0.10
  const t = 1
  const jurosEsperados = 10
  const jurosCalc = api.jurosSimples(C, i, t)
  expect(jurosCalc).toBe(jurosEsperados)
})

test('montJurosSimples', () => {
  const C = 100
  const i = 0.10
  const t = 1
  const montanteEsperado = 110
  const jurosSimples = jest.fn()
  jurosSimples.mockImplementation(() => 10)
  const montanteSimples = api.pure.montanteJurosSimples({ jurosSimples })
  const montante = montanteSimples(C, i, t)
  expect(jurosSimples.mock.calls[0]).toEqual([C, i, t])
  expect(montante).toBe(montanteEsperado)
})

test('montJurosCompostos', () => {
  const C = 1000
  const i = 0.10
  const t = 1
  const montanteEsperado = 1100
  const jurosCompostos = api.montanteJurosCompostos(C, i, t)
  expect(jurosCompostos).toBe(montanteEsperado)
})

test('jurosCompostos', () => {
  const C = 1000
  const i = 0.10
  const t = 1
  const jurosEsperados = 100
  const montanteJurosCompostos = jest.fn()
  montanteJurosCompostos.mockImplementation(() => 1100)
  const jurosCompostos = api.pure.jurosCompostos({ montanteJurosCompostos })
  const jurosCalc = jurosCompostos(C, i, t)
  expect(montanteJurosCompostos.mock.calls[0]).toEqual([C, i, t])
  expect(jurosCalc).toEqual(jurosEsperados)
})